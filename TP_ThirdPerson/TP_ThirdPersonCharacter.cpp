...
ATP_ThirdPersonCharacter::ATP_ThirdPersonCharacter()
{
	...
	P_BloodParticle = CreateDefaultSubobject<UParticleSystemComponent>(TEXT("BloodParticle"));
	P_BloodParticle->SetupAttachment(GetCapsuleComponent());
	P_BloodParticle->SetAutoActivate(false);
}
...
float ATP_ThirdPersonCharacter::TakeDamage(float DamageAmount, FDamageEvent const& DamageEvent, AController* EventInstigator, AActor* DamageCauser)
{
	...
	if (DamageEvent.IsOfType(FPointDamageEvent::ClassID))
	{
		...
		HitRotation += FRotator(-90.f, 0.f, 0.f);

		P_BloodParticle->SetWorldLocation(HitLocation);
		P_BloodParticle->SetWorldRotation(HitRotation);
		P_BloodParticle->Activate(true);
	}
